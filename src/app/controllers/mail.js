...
gerarPDF = async base64 => {
  try {
    ...
    const fileData = `${moment().format("HH-mm-DD-MM-YYYY")}.pdf`;
    const pathFile = `./src/file/${fileData}.pdf`;
    ...
    await enviarPDF(fileData, pathFile);
    console.log("done");
  } catch (error) {
    return error;
  }
...
router.post("/", async (req, res) => {
  try {
    ...
    return res.send({ mensagem: 'recebido com sucesso' });
    console.log(req);
  } catch (error) {
